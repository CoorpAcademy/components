...
const AccordionCohort = props => {
  const {tabProps, children = [], onClick = noop} = props;

  const accordionCohort = map.convert({cap: false})((child, key) => {
  ...
  return <div>{accordionCohort}</div>;
};

AccordionCohort.propTypes = {
  children: PropTypes.arrayOf(PropTypes.node),
  tabProps: PropTypes.arrayOf(PropTypes.shape(Part.PropTypes)),
  onClick: PropTypes.func
};

export default AccordionCohort;
