export default {
  props: {
    battles: [
      {
        name: 'Avatar',
        courseName: 'Les robots au service',
        avatar: 'http://minecraft.fr/wp-content/uploads/2013/05/Pa8WZj1-235x300.jpg',
        urlImg:
          'http://www.canalvie.com/polopoly_fs/1.1917585.1474313690!/image/bureau-travail.jpg_gen/derivatives/cvlandscape_670_377/bureau-travail.jpg',
        levelName: 'Base',
        onClickButtonBattle: () => console.log('test onClickButtonBattle'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      },

      {
        name: 'Homer',
        courseName: 'Les Simpson',
        avatar:
          'https://www.punanaamio.fi/media/catalog/product/cache/5/image/9df78eab33525d08d6e5fb8d27136e95/s/m/sm144-homer-simpson-julkkisnaamari.jpg',
        urlImg: 'https://img.bfmtv.com/c/1256/708/c32/01a671c64243d70195cbe64a18f2a.jpg',
        levelName: 'Coach',
        onClickButtonBattle: () => console.log('test onClickButtonBattle2'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse2'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      },

      {
        name: 'Unknown',
        courseName: 'Les robots au service',
        avatar:
          'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png',
        urlImg: 'https://consomac.fr/images/news/test-macbookair-2018-header.jpg',
        levelName: 'Advanced',
        onClickButtonBattle: () => console.log('test onClickButtonBattle'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      },

      {
        name: 'Avatar',
        courseName: 'Les robots au service',
        avatar: 'https://cdn.pixabay.com/photo/2017/06/26/02/47/people-2442565_1280.jpg',
        urlImg: 'https://bienobureau.files.wordpress.com/2014/11/bureau-travail.jpg',
        levelName: 'Base',
        onClickButtonBattle: () => console.log('test onClickButtonBattle'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      },

      {
        name: 'Avatar',
        courseName: 'Les robots au service',
        avatar: 'https://cdn.pixabay.com/photo/2017/08/30/12/45/girl-2696947_1280.jpg',
        urlImg: 'https://cdn.pixabay.com/photo/2018/02/27/09/19/colors-3185020_1280.jpg',
        levelName: 'Base',
        onClickButtonBattle: () => console.log('test onClickButtonBattle'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      },

      {
        name: 'Homer',
        courseName: 'Les Simpson',
        avatar:
          'https://www.punanaamio.fi/media/catalog/product/cache/5/image/9df78eab33525d08d6e5fb8d27136e95/s/m/sm144-homer-simpson-julkkisnaamari.jpg',
        urlImg: 'https://cdn.pixabay.com/photo/2016/11/06/05/36/landscape-1802337_1280.jpg',
        levelName: 'Coach',
        onClickButtonBattle: () => console.log('test onClickButtonBattle2'),
        onClickButtonCourse: () => console.log('test onClickButtonCourse2'),
        submitReviewCourseValue: 'Review course',
        submitStartBattleValue: 'Start battle'
      }
    ]
  }
};
