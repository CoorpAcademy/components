...
test('should add answers when action type is answer', t => {
  const result = allAnswers(config)(undefined, answerAction);
  t.deepEqual(result, [{slideRef: '1.A1.2', isCorrect: false, answer: ['foo']}]);
});
