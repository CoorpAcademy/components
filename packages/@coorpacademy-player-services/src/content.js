...
const find =
  (dataLayer: DataLayer): FindContent =>
  (type: RestrictedResourceType, ref: string): Promise<ChapterAPI | LevelAPI | Slide | void> => {
    const {findContent} = dataLayer;
    return findContent(type, ref);
  };

const getNbSlides =
  (dataLayer: DataLayer): GetNbSlides =>
  async (contentRef: string, engineRef: string, version: string): Promise<number> => {
...
const getInfo =
  (dataLayer: DataLayer): GetInfo =>
  async (contentRef: string, engineRef: string, version: string): Promise<{nbSlides: number}> => {
