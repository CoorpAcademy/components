...
const findById =
  ...
  async (id: string): Promise<Progression | void> => {
...
const getAvailableContent =
  (dataLayer: DataLayer): GetAvailableContent =>
  async (content: Content): Promise<AvailableContent> => {
...
const createSave =
  (dataLayer: DataLayer): (Progression => Promise<Progression>) =>
  (progression: Progression): Promise<Progression> => {
    const {saveProgression} = dataLayer;
    return saveProgression(progression);
  };

const findBestOf =
  (dataLayer: DataLayer): FindBestOf =>
  async (
    engineRef: string,
    contentType: ContentType,
    contentRef: string,
...
const addActionAndSaveProgression =
  (dataLayer: DataLayer): ((Progression, Action) => Promise<Progression>) =>
  (progression: Progression, action: Action): Promise<Progression> => {
...
const postAnswer =
  (dataLayer: DataLayer): PostAnswer =>
  async (
    progressionId: string,
    payload: UserAnswerAPI,
...
const requestClue =
  (dataLayer: DataLayer): RequestClue =>
  async (progressionId: string, payload: {content: ContentSlide}): Promise<Progression> => {
    const {findProgressionById} = dataLayer;
    const progression = await findProgressionById(progressionId);
...
const acceptExtraLife =
  (dataLayer: DataLayer): AcceptExtraLife =>
  async (
    progressionId: string,
    payload: {
      content: Content
...
const refuseExtraLife =
  (dataLayer: DataLayer): RefuseExtraLife =>
  async (
    progressionId: string,
    payload: {
      content: Content
...
const create =
  (dataLayer: DataLayer): CreateProgression =>
  async (
    _id: string,
    engine: Engine,
...
const markResourceAsViewed =
  (dataLayer: DataLayer): MarkResourceAsViewed =>
  async (
    progressionId: string,
    payload: {
      resource: ResourceContent,
