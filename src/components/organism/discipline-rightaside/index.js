    ...
    return (
        <div className={style.col}>
          <div className={style.head}>
            ...
            <div className={style.category}>{product.category.value}</div>
          </div>
          
          <a href="https://store-staging.coorpacademy.com/checkout/cart/{product.sku}" className={style.buyall}><span>acheter</span> ce cours</a>
          <a href="#" className={style.buysample}>obtenir un <span>essai</span> pour 1.99€</a>

          <div className={style.author}>auteur</div>
          <div className={style.title}>{product.author}</div>
        </div>
      );
  };
